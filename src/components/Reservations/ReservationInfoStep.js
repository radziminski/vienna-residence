...
export class ReservationsStartForm extends Component {
    ...
    // Component Lifecycle
    componentDidMount() {
        if (this.props.dates.checkInDate) this.setState({ checkInDate: this.props.dates.checkInDate });

        if (this.props.dates.checkOutDate) this.setState({ checkOutDate: this.props.dates.checkOutDate });

        if (this.props.people.adultsNum) this.setState({ adultsNum: this.props.people.adultsNum });

        if (this.props.people.childrenNum) this.setState({ childrenNum: this.props.people.childrenNum });

        if (this.props.people.babiesNum) this.setState({ babiesNum: this.props.people.babiesNum });

        if (this.props.premium) this.setState({premium: this.props.premium});
    }
    ...
    componentDidUpdate() {
        ...
        if (this.state.premium === 'notSet') {
            this.setState({ premium: this.props.premium === true})
        }
        ...
        if (this.state.checkInDate && this.state.checkOutDate) {
            ...
            if (dateDifference !== this.state.hotelNights) this.setState({ hotelNights: dateDifference });
    ...
    render() {
        ...
        let summary = this.state.hotelNights
            ? `${this.state.hotelNights} Hotel Nights\t`
            : '(Please choose reservation dates)\t';
        ...
        if (this.state.btnEnabled) {
            sendBtn = (
                <div className={this.classNames.element('btn')}>
                    <Button onClick={this.onNextStepHandler} type="full" width="100%">
                        Check Availability
                    </Button>
                </div>
            );
            ...
            <div className={this.classNames.block()}>
                <div className={this.classNames.element('dates-wrapper')}>
                    <div className={this.classNames.element('date-wrapper')}>
                        ...
                        <div
                            className={this.classNames.element('date-input')}
                ...
                <div className={this.classNames.element('premium')}>
                    <ToggleCheckbox onChange={this.onToggleChange} checked={this.state.premium} />
                    <div className={this.classNames.element('premium-text')}>Only premium experiance</div>
                </div>
